...
export default handler(async function (req, res) {
  ...
  if (req.method === 'POST') {
    ...
    await sql`
      ...
      values (${medId}, ${t.patient_id}, ${date}, ${b.status}, ${hhmm(b.scheduledTime)}, ${hhmm(b.actualTime)},
              ${t.name}, ${u.id}, ${Date.now()})
      on conflict (treatment_id, date) do update set status = excluded.status, scheduled_time = excluded.scheduled_time,
        actual_time = excluded.actual_time, recorded_by = excluded.recorded_by, ts = excluded.ts`;
    return res.json({ ok: true });
  }
  if (req.method === 'DELETE') {
    await sql`delete from adherence where treatment_id = ${medId} and date = ${date}`;
    return res.json({ ok: true });
  }
