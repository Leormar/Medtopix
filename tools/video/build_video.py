#!/usr/bin/env python3
"""Pipeline del video MedTopix: escenas HTML -> PNG (Chrome headless) -> voz edge-tts
-> música sintetizada (numpy) con ducking -> segmentos zoompan -> mp4.
Uso: venv/bin/python build_video.py   (env: VOZ, RATE, OUT, STAGE=png|all)"""
import os, subprocess, sys, json, wave
import numpy as np
from scenes import SCENES, SPLIT, page

HERE = os.path.dirname(os.path.abspath(__file__))
VOZ = os.environ.get('VOZ', 'es-CO-SalomeNeural')
RATE = os.environ.get('RATE', '+3%')
OUT = os.environ.get('OUT', '/Users/leo/Medtopix/video/medtopix.mp4')
POSTER = os.environ.get('POSTER', '/Users/leo/Medtopix/img/video-poster.jpg')
...
for sid, text, _ in SCENES:
    ...
    if not os.path.exists(mp3):
        run([os.path.join(HERE, 'venv/bin/edge-tts'), '--voice', VOZ, '--rate=' + RATE, '--text', text, '--write-media', mp3])
    durs.append(dur(mp3))
...
run(['ffmpeg', '-y', '-i', os.path.join(HERE, 'png', os.environ.get('POSTER_SHOT', 's7_0') + '.png'), '-vf', 'scale=1920:1080', '-q:v', '3', POSTER])
